...
static inline void button2SetTextColour (TCCBUTTON2 *button, const int idx, const unsigned int fore, const unsigned int back, const unsigned int outline)
{
	TBUTTON2LABEL *bl = buttonLabelGet(button, idx);
	if (bl)
...
static inline int button2SetText (TCCBUTTON2 *button, const int idx, const char *str, const int flags, const int font, const int x, const int y)
{
	TBUTTON2LABEL *bl = buttonLabelGet(button, idx);
	if (bl){
...
static inline int button2CreateFaceImgId (TCCBUTTON2 *button, const int priId, const int secId, double scale, const int x, const int y)
{
	...
	if (ret){
		...
		if (!artManagerImageGetMetrics(button->cc->vp->am, priId, &width, &height)){
			printf("can not access image %X\n", priId);
			if (!artManagerImageGetMetrics(button->cc->vp->am, secId, &width, &height)){
				printf("can not access image %X\n", secId);
				return 0;
			}
		}
...
int button2FaceTextSet (TCCBUTTON2 *button, const int idx, const char *str, const int flags, const int font, const int x, const int y)
{
	int ret = 0;
	if (ccLock(button)){
