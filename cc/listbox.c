...
void listboxSetLineHeight (TLISTBOX *listbox, const int lineHeight)
{
	if (lineHeight){
		...
		listbox->pane->vertLineHeight = listbox->flags.lineHeight;
	}
}

void listboxSetFont (TLISTBOX *listbox, const int font)
{
	if (font)
		listbox->flags.font = font;
}
...
int listboxUpdateItemEx (TLISTBOX *listbox, const int itemId, const char *string, const int font, const int newImgId)
{	
	int ret = 0;
...
void labelSetItemFont (TLISTBOX *listbox, const int itemId, const int font)
{
	if (itemId)
		labelRenderFontSet(listboxGetBase(listbox), itemId, font);
}
...
int listboxRender (void *object, TFRAME *frame)
{
	...
	if (listbox->underlay.imgId && listbox->underlay.drawShadow){
		...
		drawShadowUnderlay(listbox->cc->vp, frame, metrics.x-1, metrics.y-1, metrics.width+2, metrics.height+2, listbox->underlay.colour);
	}
...
int listboxNew (TCCOBJECT *object, void *unused, const int pageOwner, const int type, const TCommonCrtlCbMsg_t lb_cb, int *id, const int width, const int height)
{
	...
	listbox->flags.font = LISTBOX_FONT_DEFAULT;
	listbox->flags.lineHeight = LISTBOX_LINEHEIGHT_DEFAULT;
	listbox->flags.imgScale = (double)listbox->flags.lineHeight / (double)listbox->cc->vp->gui.artMaxHeight;
	...
	pane->vertLineHeight = listbox->flags.lineHeight;
