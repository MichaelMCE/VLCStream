...
#if (MXML_CAN_SAX_LOAD || MXML_CAN_LOAD)
NTSTATUS
NTAPI
NtReadFile(
  IN HANDLE FileHandle,
...
static inline int		mxml_add_char(int ch, char **ptr, char **buffer, int *bufsize);
static inline int		mxml_fd_getc(void *p, int *encoding);
//static inline int mxml_fd_putc(int ch, void *p);
static inline int		mxml_fd_read(_mxml_fdbuf_t *buf);
//static inline int		mxml_fd_write(_mxml_fdbuf_t *buf);
static inline int		mxml_file_getc(void *p, int *encoding);
//static inline int		mxml_file_putc(int ch, void *p);
static inline int		mxml_get_entity(mxml_node_t *parent, void *p, int *encoding, _mxml_getc_cb_t getc_cb);
...
static inline int		mxml_parse_element(mxml_node_t *node, void *p, int *encoding, _mxml_getc_cb_t getc_cb);
static inline int		mxml_string_getc(void *p, int *encoding);
//static inline int		mxml_string_putc(int ch, void *p);
//static inline int		mxml_write_name(const char *s, void *p, _mxml_putc_cb_t putc_cb);
//static inline int		mxml_write_node(mxml_node_t *node, void *p, mxml_save_cb_t cb, int col, _mxml_putc_cb_t putc_cb, _mxml_global_t *global);
//static inline int		mxml_write_string(const char *s, void *p, _mxml_putc_cb_t putc_cb);
//static inline int		mxml_write_ws(mxml_node_t *node, void *p,  mxml_save_cb_t cb, int ws, int col, _mxml_putc_cb_t putc_cb);
    ...
    mxml_save_cb_t cb)			/* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  */
  ...
  if (bytes < (int)(sizeof(buffer) - 1))
  {
    ...
    */
    ...
    return (duplicateBytesZSafe(buffer));
  }
  ...
  */
  ...
  if ((s = allocateSafe(bytes + 1)) == NULL)
    return (NULL);
...
static inline int				/* O  - 0 on success, -1 on error */
	      ...
	      int  *bufsize)		/* IO - Current buffer size */
{
  ...
  if (*bufptr >= (*buffer + *bufsize - 4))
  {
    ...
    */
    ...
    if ((newbuffer = reAllocateSafe(*buffer, *bufsize)) == NULL)
    {
      my_free(*buffer);
...
static inline int				/* O  - Character or EOF */
...
static inline int				/* O  - Character value or EOF on error */
    ...
    void            *sax_data)		/* I - SAX user data */
{
  ...
  */
  ...
  if ((buffer = allocateSafe(64)) == NULL)
  {
  ...
  while ((ch = (*getc_cb)(p, &encoding)) != EOF)
  {
	...
	case MXML_INTEGER :
            ...
            node = mxmlNewInteger(parent, _atoi64(buffer));
...
static inline int				/* O  - Terminating character */
mxml_parse_element(
...
static inline int				/* O - 0 on success, -1 on failure */
...
static inline int				/* O - Column or -1 on error */
    ...
    switch (node->type)
    {
	  ...
	  if (node->prev)
	  {
	    if (global->wrap > 0 && col > global->wrap)
	    {
          ...
          sprintf(s, "%I64d", node->value.integer);
...
static inline int				/* O - New column */
