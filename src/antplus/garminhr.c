...
#if 0
static inline void dumpPayload (const unsigned char *payLoad, const size_t dataLength)
{
	printf("payload { ");
...
int channelEvent (const int chan, const int eventId, const unsigned char *payLoad, const size_t dataLength, void *uPtr)
{
	...
	switch (eventId){
	  case EVENT_RX_SEARCH_TIMEOUT:
	  	dbprintf(hr->vp, " $ event RX search timeout");
	  	break;
	  ...
	  case EVENT_RX_BROADCAST:
		...
		if (hr->rate->currentSequence != hr->rate->previousSequence){
			...
			if (hr->rate->currentBpm > hr->rate->high)
	  		...
	  		if (hr->pulseCB) hr->pulseCB(hr->rate, hr->pulsePtr);
	  	}
...
int responseEvent (const int chan, const int msgId, const unsigned char *payLoad, const size_t dataLength, void *uPtr)
{
	...
	switch (msgId){
	  ...
	  case MESG_NETWORK_KEY_ID: 
	  	//dbprintf(hr->vp, " * network key accepted");
	  	// master
	  ...
	  case MESG_OPEN_CHANNEL_ID:
 	  ...
 	  case MESG_UNASSIGN_CHANNEL_ID:
		dbprintf(hr->vp, " * channel Unassigned");
		break;

	  case MESG_CLOSE_CHANNEL_ID:
		dbprintf(hr->vp, " * channel closed");
		hr->cstates->channelStatus = 0;
		break;
		
 	  case CHANNEL_IN_WRONG_STATE:          
		dbprintf(hr->vp, " * channel in wrong state");
		break;

 	  case CHANNEL_NOT_OPENED:              
		dbprintf(hr->vp, " * channel not opened");
		break;
		
 	  case CHANNEL_ID_NOT_SET: //??             
		dbprintf(hr->vp, " * channel ID not set");
		break;
		
 	  case CLOSE_ALL_CHANNELS: // Start RX Scan mode              
		dbprintf(hr->vp, " * close all channels");
		break;
		
 	  case TRANSFER_IN_PROGRESS: // TO ack message ID            
		dbprintf(hr->vp, " * tranfer in progress");
		break;
		
 	  case TRANSFER_SEQUENCE_NUMBER_ERROR:  
		dbprintf(hr->vp, " * transfer sequence number error");
		break;
		
 	  case TRANSFER_IN_ERROR:               
		dbprintf(hr->vp, " * transfer in error");
		break;
		
 	  case INVALID_MESSAGE:                 
		dbprintf(hr->vp, " * invalid message");
		break;
		
 	  case INVALID_NETWORK_NUMBER:          
		dbprintf(hr->vp, " * invalid network number");
		break;
		
 	  case INVALID_LIST_ID:                 
		dbprintf(hr->vp, " * invalid list ID");
		break;
		
 	  case INVALID_SCAN_TX_CHANNEL:         
		dbprintf(hr->vp, " * invalid Scanning transmit channel");
		break;
		
 	  case INVALID_PARAMETER_PROVIDED:      
		dbprintf(hr->vp, " * invalid parameter provided");
   		break;
   		
 	  case EVENT_QUE_OVERFLOW:              
		dbprintf(hr->vp, " * queue overflow");
		break;
		
	  default:
	  	dbprintf(hr->vp, " #### unhandled response id %i", msgId);
	};
...
int messageEventCb (const int chan, const int msgId, const unsigned char *payLoad, const size_t dataLength, void *uPtr)
{
	...
	switch(msgId){
	  ...
	  case MESG_STARTUP_MESG_ID: 
	  	//dbprintf(hr->vp, " @ start up mesg reason: 0x%X", payLoad[STREAM_STARTUP_REASON]);
		libantplus_SetNetworkKey(ant, hr->dcfg->networkNumber, hr->dcfg->dongle.key);
		hr->keyset = 0;
		break;
	  ...
	  case MESG_CHANNEL_STATUS_ID:{
	  	//dbprintf(hr->vp, " @ channel status for channel %i is %i", payLoad[STREAM_CHANNEL_ID], payLoad[STREAM_CHANNEL_STATUS]);
		int ucStatusByte = payLoad[STREAM_CHANNELSTATUS_STATUS] & STATUS_CHANNEL_STATE_MASK;
		dbprintf(hr->vp, "STATUS: %s",astrStatus[ucStatusByte]);
		
	  	break;
	  } 	
	  case MESG_CAPABILITIES_ID:
		//dbprintf(hr->vp, " @ capabilities:");
		dbprintf(hr->vp, "   Max ANT Channels: %i",payLoad[STREAM_CAP_MAXCHANNELS]);
		dbprintf(hr->vp, "   Max ANT Networks: %i",payLoad[STREAM_CAP_MAXNETWORKS]);
		dbprintf(hr->vp, "   Std. option: 0x%X",payLoad[STREAM_CAP_STDOPTIONS]);
		dbprintf(hr->vp, "   Advanced: 0x%X",payLoad[STREAM_CAP_ADVANCED]);
		dbprintf(hr->vp, "   Advanced2: 0x%X",payLoad[STREAM_CAP_ADVANCED2]);
	  	break;
	...
	case MESG_CHANNEL_ID_ID:
		...
		dbprintf(hr->vp, " @ CHANNEL ID: (%i/%i/%i)", hr->dev->scidDeviceNumber, hr->dev->scidDeviceType, hr->dev->scidTransType);        
         
		if (hr->dev->scidDeviceType != hr->dcfg->deviceType){
			dbprintf(hr->vp, " @ CHANNEL ID: this is not the device we're looking for");
			dbprintf(hr->vp, " @ CHANNEL ID: expecting 0x%X but found 0x%X", hr->dcfg->deviceType, hr->dev->scidDeviceType);
	...
	case MESG_VERSION_ID:
		dbprintf(hr->vp, " @ version: '%s'", (char*)&payLoad[STREAM_VERSION_STRING]);
		break;
	};
