...
// libantplus - https://github.com/MichaelMCE/
...
static inline unsigned char calc_checksum (const unsigned char *restrict buffer, const unsigned char len){
...
static inline int antstick_getCount (const int vid, const int pid)
{
...
int libantplus_GetDeviceCount (const int vid, const int pid)
{
    usb_find_busses();
    usb_find_devices();
	return antstick_getCount(vid, pid);
}
...
TLIBANTPLUS *libantplus_Init (const int vid, const int pid)
{
	usb_init();
    usb_find_busses();
    usb_find_devices();

	TLIBANTPLUS *ant = (TLIBANTPLUS*)my_calloc(1, sizeof(TLIBANTPLUS));
	if (ant){
		ant->initOnce = 1;
		ant->iDevice  = -1;
		ant->vid = vid;
