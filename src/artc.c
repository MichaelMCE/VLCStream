...
//  but WITHOUT ANY WARRANTY; without even the amplied warranty of
...
static inline int hashTableFind (TARTMANAGER *am, const unsigned int hash)
{
...
static inline void hashTableAdd (TARTMANAGER *am, const unsigned int hash, const int id)
{
	...
	if (ai->state&ARTC_STATE_HASPATH /*&& ai->image.type != IMG_BMP*/){
		...
		if (surface){
			if (ai->state&ARTC_STATE_ISRESIZED){
				...
				if (tmp){
					if (lResizeFrame(surface, ai->resize.width, ai->resize.height, 0) == 1){
						copyAreaScaled(tmp, surface, 0, 0, tmp->width, tmp->height, 0, 0, ai->resize.width, ai->resize.height);
			...
			if (ai->state&ARTC_STATE_HASCUSTMETRICS){
				...
				imageBestFit(ai->bestfit.width, ai->bestfit.height, surface->width, surface->height, &ai->image.width, &ai->image.height);
...
static inline int artManagerItemGetMetrics (TART_ITEM *ai, int *width, int *height)
{
	...
	if (!(ai->state&ARTC_STATE_HASMETRICS)){
		if (artManagerItemReadMetrics(ai)){
			if (ai->state&ARTC_STATE_HASCUSTMETRICS){
				int w, h;
				imageBestFit(ai->bestfit.width, ai->bestfit.height, ai->image.width, ai->image.height, &w, &h);
				ai->image.width = w;
				ai->image.height = h;
			}
		}
		...
		if (artManagerImageAcquire(am, id)){
			...
			if (tmp){
				if (lResizeFrame(ai->surface, width, height, 0) == 1){
					copyAreaScaled(tmp, ai->surface, 0, 0, tmp->width, tmp->height, 0, 0, width, height);
				...
				if (as){
					...
					if (as->surface){
						if (opacity == 100 || opacity <= 0){
							...
							drawImageScaledOpacity(ai->surface, as->surface, 0, 0, ai->surface->width, ai->surface->height, 0, 0, w, h, opacityA, opacityA*0.65f);
						}
...
{
	const unsigned int hash = getHashW(path);
	for (int i = 0; am->hashTable[i].hash && i < am->hashTableSize; i++){
		if (am->hashTable[i].hash == hash){
			am->hashTable[i].id = 0;
			am->hashTable[i].hash = 0;
...
static inline int artManagerFindHash (TARTMANAGER *am, const unsigned int hash)
{
...
TARTMANAGER *artManagerNew (THWD *hw)
{
