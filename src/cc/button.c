...
static inline void buttonSetTextColour (TCCBUTTON *button, const int idx, const unsigned int fore, const unsigned int back, const unsigned int outline)
{
...
static inline int buttonSetText (TCCBUTTON *button, const int idx, const char *str, const int flags, const int font, const int x, const int y)
{
	TBUTTONLABEL *bl = button->btnlbl[idx];
	if (bl){
...
void buttonBorderProfileSet (TCCBUTTON *button, const int set, const unsigned int *colour, const int total)
{
	if (ccLock(button)){
		if (button->active)
...
void buttonBaseColourSet (TCCBUTTON *button, const unsigned int colour)
{
	if (ccLock(button)){
		if (button->active)
...
static inline int buttonCreateFaceImgId (TCCBUTTON *button, const int priId, const int secId, const int x, const int y)
{
	...
	if (ret){
		...
		if (!imageManagerImageGetMetrics(button->cc->vp->im, priId, &width, &height)){
			if (secId && !imageManagerImageGetMetrics(button->cc->vp->im, secId, &width, &height))
				return 0;
		}
...
int buttonFacePathSet (TCCBUTTON *btn, wchar_t *pri, wchar_t *sec, const int x, const int y)
{
	...
	if (ccLock(btn)){
		...
		if (pri)
			priId = imageManagerImageAdd(btn->cc->vp->im, pri);

		if (sec)
			secId = imageManagerImageAdd(btn->cc->vp->im, sec);
...
int buttonFaceTextSet (TCCBUTTON *button, const int idx, const char *str, const int flags, const int font, const int x, const int y)
{
	int ret = 0;
	if (ccLock(button)){
...
void buttonFaceTextColourSet (TCCBUTTON *button, const int face, const unsigned int fore, const unsigned int back, const unsigned int outline)
{
	if (ccLock(button)){
...
int buttonFaceHoverSet (TCCBUTTON *button, const int state, const unsigned int colour, const double alpha)
{
	int ret = 0;
	if (ccLock(button)){
		ret = button->flags.canHover;
...
unsigned int buttonsRenderAll (TCCBUTTONS *btns, TFRAME *frame, const int flags)
{
	...
	const int64_t dt = getTickCount() - btns->t0;
...
unsigned int buttonsRenderAllEx (TCCBUTTONS *btns, TFRAME *frame, const int flags, const int cx, const int cy)
{
	if (!btns || !btns->total) return 0;
...
static inline int ccbuttonRender (void *object, TFRAME *frame)
{
	...
	unsigned int rflags = labelP->renderflags;
