...
static inline void drawLine (TFRAME *restrict frame, const int x1, const int y1, const int x2, const int y2, const int colour)
{
...
static inline void graphSheetRender_Scope (TGRAPHSHEET *sheet, TFRAME *frame, TMETRICS *metrics)
{
	...
	if (sheet->render.mode&GRAPH_AUTOSCALE){
		...
		if (!sheet->stats.ceiling){
			range = abs(sheet->stats.max - sheet->stats.min)+1.0;
		}else{
	...
	if (sheet->render.hints&GRAPH_HINT_SCOPE_FILL){
		...
		for (int x = metrics->x; x < metrics->x + metrics->width; x++){
			if ((int)x != lastX){
				...
				drawLine(frame, x, y1, x, btmY, col);
				lastX = x;
			}
		}
#else
	...
	if (sheet->render.hints&(GRAPH_HINT_SHADOW1|GRAPH_HINT_SHADOW2)){
		...
		for (int i = 0; i < total-1; i++, x += scaleX){
			...
			if (sheet->render.hints&GRAPH_HINT_SHADOW1){
				drawLine(frame, x-1.0, y1, x+scaleX-1.0, y2, colSdw1);	// w
				drawLine(frame, x+1.0, y1, x+scaleX+1.0, y2, colSdw1);	// e
				drawLine(frame, x, y1-1.0, x+scaleX, y2-1.0, colSdw1);	// n
				drawLine(frame, x, y1+1.0, x+scaleX, y2+1.0, colSdw1);	// s
			}
			if (sheet->render.hints&GRAPH_HINT_SHADOW2){
				drawLine(frame, x-1.0, y1-1, x+scaleX-1.0, y2-1, colSdw2);	// nw
				drawLine(frame, x+1.0, y1+1, x+scaleX+1.0, y2+1, colSdw2);	// se
			}
		}
	}
...
static inline void graphSheetRender_Polyline (TGRAPHSHEET *sheet, TFRAME *frame, TMETRICS *metrics)
{
	...
	if (sheet->render.hints&(GRAPH_HINT_SHADOW1|GRAPH_HINT_SHADOW2)){
		...
		for (int i = firstPt; i < total-1; i++, x++){
			...
			if (sheet->render.hints&GRAPH_HINT_SHADOW1){
				drawLine(frame, x-1, y1, x+1-1, y2, colSdw1);		// left
				drawLine(frame, x+1, y1, x+1+1, y2, colSdw1);		// right
				drawLine(frame, x, y1-1, x+1, y2-1, colSdw1);		// up
				drawLine(frame, x, y1+1, x+1, y2+1, colSdw1);		// down
			}
			if (sheet->render.hints&GRAPH_HINT_SHADOW2){
				drawLine(frame, x-1, y1-1, x+1-1, y2-1, colSdw2);	// nw
				drawLine(frame, x+1, y1+1, x+1+1, y2+1, colSdw2);	// se
			}
		}
		x = firstX;
	}
