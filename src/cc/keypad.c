...
static inline TKP_PAD *keypadCreatePad (TKEYPAD *kp, const TPOINT *pos)
{
...
static inline int keypadAddButton (TKEYPAD *kp, const int padId, const int keyType, const UTF32 codePoint, const TPOINT *pos, TCCBUTTON2 *btn)
{
...
static inline int keypadAddKey (TKEYPAD *kp, const int padId, const int keyType, const TPOINT *posOffset, const int canAnimate, UTF32 codePoint, wchar_t *imageFacePath)
{
	...
	if (btn){
		...
		const int imgId = artManagerImageAdd(kp->cc->vp->am, buildSkinD(kp->cc->vp, buffer, imageFacePath));
...
int keypadEditboxCaretMoveEnd (TKP_EDITBOX *eb)
{
	...
	const int cspace = lGetFontCharacterSpacing(label->cc->vp->ml->hw, KEYPAD_INPUT_FONT);
	TWCHAR *glyphA = lGetGlyph(label->cc->vp->ml->hw, NULL, L'a', KEYPAD_INPUT_FONT);
	if (!glyphA) return 0;
...
int keypadEditboxCaretMoveRight (TKP_EDITBOX *eb)
{
	...
	TWCHAR *glyphSpace = lGetGlyph(label->cc->vp->ml->hw, NULL, L' ', KEYPAD_INPUT_FONT);
	if (glyphSpace)
...
static inline int64_t keybtn_cb (const void *object, const int msg, const int64_t data1, const int64_t data2, void *dataPtr)
{
	...
	if (msg == BUTTON_MSG_SELECTED_PRESS){
		...
		kp->lastKeyPressTime0 = getTickCount();
		...
		if (key->type == KP_KEYS_BACKSPACE){
			...
			kp->lastKeyReleaseTime = getTime(btn->cc->vp);
		}
...
{	
	...
	kp->editbox.labelId = labelTextCreate(kp->editbox.label, " ", /*PF_TEXTBOUNDINGBOX|PF_GLYPHBOUNDINGBOX|*/PF_LEFTJUSTIFY, KEYPAD_INPUT_FONT, 0, 2);
...
int keypadNew (TCCOBJECT *object, void *unused, const int pageOwner, const int type, const TCommonCrtlCbMsg_t tv_cb, int *id, const int x, const int y)
{
	...
	kp->metrics.x = x;
	kp->metrics.y = y;
	kp->metrics.width = kp->cc->vp->ml->width - kp->metrics.x;		// fix this
	kp->metrics.height = kp->cc->vp->ml->height - kp->metrics.y;
	kp->canDrag = 0;
	kp->tPads = 0;
	kp->isBuilt = 0;
