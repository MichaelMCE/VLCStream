...
int genAMId (TPANEL *panel, wchar_t *path)
{
	...
	return artManagerImageAdd(panel->cc->vp->am, buildSkinD(panel->cc->vp,buffer,path));
}
	...
	if (panel->itemTotal){
		...
		if (item){
			if (!item->isActive){
				...
				const int hoverCol = swatchGetColour(panel->cc->vp, PAGE_NONE, SWH_PLY_ISELECTED);
				button2FaceHoverSet(item->btn, 1, hoverCol, 0.8);
				//if (artIdAlt)
...
int panelInvalidate (TPANEL *panel)
{
	...
	panelImgTextMetricsCalc(panel, panel->cc->vp->ml->hw, panel->font, panel->list, panel->listSize);
...
int panelRender (void *object, TFRAME *frame)
{
				...
				for (int i = 0; i < PANEL_ITEM_LABELTOTAL; i++){
					...
					if (posY + label->metrics.y + 24 >= constraint.y1 && posY + label->metrics.y <= constraint.y2){
						if (!label->strFrm){
							...
							label->strFrm = newStringEx(frame->hw, &metrics, LFRM_BPP_32A, 0, label->font, label->text, panel->itemMaxWidth, NSEX_RIGHT);
						}
						...
						if (label->strFrm){
							...
							T4POINT pos = {posX+label->metrics.x, posY+label->metrics.y, posX+label->metrics.x+text->width-1, posY+label->metrics.y+text->height-1};
	...
	const unsigned int dt = buttonsRenderAll(panel->btns, frame, BUTTONS_RENDER_HOVER|BUTTONS_RENDER_ANIMATE);
	if (swipe->decayAdjust > 0.0 || dt < 1000)
		setTargetRate(panel->cc->vp, 25);
	else
		setTargetRate(panel->cc->vp, UPDATERATE_BASE);
...
int panelInput (void *object, TTOUCHCOORD *pos, const int flags)
{
	...
	if (panel->dragEnabled){
		if (!pos->pen && flags == 0 && pos->dt > 80){			// pen down
			...
			if (dt && abs(swipe->dy) >= swipe->dragMinV){
			...
			swipe->dt = getTime(panel->cc->vp) - swipe->t0;
			if (swipe->dt > 0.0){
				double velocity = swipe->dy / swipe->dt;
				swipe->velocity = velocity;
			}
...
int panelNew (TCCOBJECT *object, void *unused, const int pageOwner, const int type, const TCommonCrtlCbMsg_t panel_cb, int *id, const int buttonTotal, const int var2)
{
	...
	//panel->itemOffset->x = 0;	// offset each and every image location by T2POINT:x/y
	//panel->itemOffset->y = 0;
	panel->itemMaxWidth = getFrontBuffer(panel->cc->vp)->width * 0.27;
