...
static inline void sliderBuildMidHori (TSLIDER *slider, TLABEL *label, const int width, const int height)
{
	...
	TFRAME *img = lNewFrame(slider->cc->vp->ml->hw, midWidth, midHeight, src->bpp);
	const int x2 = src->width-1;
	
	for (int x = 0; x < img->width-1; x += src->width){
		if (x+x2 < img->width)
			copyAreaNoBlend(src, img, x, 0, 0, 0, x2, src->height-1);
		else
			copyAreaNoBlend(src, img, x, 0, 0, 0, (img->width%src->width)-1, src->height-1);
	}
...
static inline void sliderBuildMidVert (TSLIDER *slider, TLABEL *label, const int width, const int height)
{
	...
	for (int y = 0; y < img->height; y += src->height){
		if (y+y2 < img->height)
			copyAreaNoBlend(src, img, 0, y, 0, 0, src->width-1, y2);
		else
			copyAreaNoBlend(src, img, 0, y, 0, 0, src->width-1, (img->height%src->height)-1);
	}
...
int sliderFaceSet (TSLIDER *slider, int face, wchar_t *path)
{
	...
	if (ccLock(slider)){
		if (face == SLIDER_FACE_MID || face == SLIDER_FACE_MIDSRC){
			...
			TFRAME *img = newImage(slider->cc->vp, buildSkinD(slider->cc->vp, buffer, path), SKINFILEBPP);
			if (img){
...
int sliderNew (TCCOBJECT *object, void *unused, const int pageOwner, const int sliderType, const TCommonCrtlCbMsg_t slider_cb, int *id, const int width, const int height)
{
	...
	int lblid;
	slider->base = ccCreate(slider->cc->vp->cc, pageOwner, CC_LABEL, slider_label_cb, &lblid, width, height);
	...
	TFRAME *img = lNewFrame(slider->cc->vp->ml->hw, 8, 8, SKINFILEBPP);
	if (img){
		...
		const int imgStubId = imageManagerImageAdd(slider->cc->vp->im, L"cc\\stub.png");
