...
static inline void drawRectangleFilledConstrained (TFRAME *frame, const int x1, const int y1, const int x2, const int y2, const int colour, const int miny, const int maxy)
{
...
static inline TTV_RENDER_ITEM *tvRenderLabel (TTV *tv, TFRAME *frame, TTV_ITEM *item, TLPRINTR *rt)
{
	...
	unsigned int shash = getHash(str) ^ (tv->renderFlags<<24 ^ frame->hw->render->foreGround);
	TFRAME *img = strcFindString(tv->cc->vp->strc, shash);
	if (!img){
		TMETRICS metrics = {1, 5, 0, 0};
		img = newStringEx(frame->hw, &metrics, LFRM_BPP_32A, flags, font, str, w, NSEX_LEFT);
		strcAddString(tv->cc->vp->strc, img, shash);
	}
	...
	if (img){
		...
		if (tv->renderFlags&TV_RENDER_LABELS){
			if (tv->renderFlags&TV_RENDER_LABELROWHL){
				if (item->id&1)
					drawRectangleFilledConstrained(frame, post->pos.x1, post->pos.y1+1, post->pos.x1+img->width-1, post->pos.y1+h-y1-1, 25<<24|0x222222, 0, frame->height-1);
				else
					drawRectangleFilledConstrained(frame, post->pos.x1, post->pos.y1+1, post->pos.x1+img->width-1, post->pos.y1+h-y1-1, 25<<24|0x121262, 0, frame->height-1);
			}
			copyArea(img, frame, post->pos.x1, post->pos.y1, 0, y1, img->width-1, h);
		}
...
int _tvRender (TTV *tv, TFRAME *frame, TTV_RENDER *restrict pre)
{
	if (tv->renderFlags&TV_RENDER_BLUR){
		...
		//drawRectangleFilled(frame, &tv->metrics, 100<<24|COL_BLACK);
	}
	if (tv->renderFlags&TV_RENDER_UNDERLAY)
		drawRectangleFilled(frame, &tv->metrics, (60<<24)|COL_BLUE_SEA_TINT);
		
	if (tv->renderFlags&TV_RENDER_AREAFILLED)
		drawRectangleFilled(frame, &tv->metrics, (80<<24)|COL_BLACK);
...
void tvRenderSetNodeIdFont (TTV *tv, const int font)
{
	tv->idt.font = font;
}
...
int tvRender (void *object, TFRAME *frame)
{
	...
	if (ccLock(tv)){
		...
		if (tv->renderFlags&TV_RENDER_IDTEXT && tv->cc->cursor->isHooked && tv->inputEnabled){
			TTOUCHCOORD pos = {.x = tv->cc->vp->gui.cursor.dx, .y = tv->cc->vp->gui.cursor.dy};

			int id = tvInputGetUnderCursor(tv, tv->postRender, &pos);
			if (id)
				drawHex(frame, tv->idt.pos.x, tv->idt.pos.y, tv->idt.font, id, tv->idt.colour);
		}
...
int tvHandleInput (void *object, TTOUCHCOORD *pos, const int flags)
{
	...
	if (ccLock(tv)){
		if (tv->renameEnabled && !flags){		// we're renaming an item, find then setup a keypad callback
			...
			if (item){
				//if (pageGetSec(vp) != PAGE_VKEYBOARD){
