...
static int skinReloadBackground (TVLCPLAYER *vp, TGUI *gui, const int randBack)
{
	...
	if (strList){
		...
		if (randBack == 1){
			...
			if (--gui->skin.currentIdx < 0)
				gui->skin.currentIdx =  gui->skin.bgPathTotal-1;
		}
...
static void doRotary (TCFG *cfg, rotary_t *enc)
{
	if (enc->encoderId == 1){
		TVLCPLAYER *vp = (TVLCPLAYER*)cfg->com->vp;
		
		if (enc->positionDelta != 0){
			if (renderLock(vp)){
				if (vp->applState){
					if (enc->positionDelta > 0)
						skinReloadBackground(vp, &vp->gui, 1);
					else
						skinReloadBackground(vp, &vp->gui, 2);
					imageManagerImageRelease(vp->im, vp->gui.image[IMGC_BGIMAGE]);
			...
			page2SetPrevious(cfg);
		}
	}
}

static inline int page_cfgInput (TCFG *cfg, TVLCPLAYER *vp, const int msg, const int flags, void *dataPtr)
{
	...
	if (msg == PAGE_IN_WHEEL_FORWARD){
  	...
  	}else if (msg == PAGE_IN_ROTARY_CHANGE){	
	  	doRotary(cfg, (rotary_t*)dataPtr);
	}
