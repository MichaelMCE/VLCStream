...
static inline int page_clkRenderBegin (TCLK *clk, TVLCPLAYER *vp, int64_t time0, int64_t zDepth, TFRAME *frame, void *opaquePtr)
{
	...
	if (clk->displayType == CLOCK_ANALOGUE){
		...
		if (!(clk->analogue.face && clk->analogue.hr && clk->analogue.min && clk->analogue.sec && clk->analogue.cap))
			return 0;
		...
		if (!(clk->butterfly.face && clk->butterfly.hr && clk->butterfly.min && clk->butterfly.sec && clk->butterfly.cap))
			return 0;
    	...
    	for (int i = 0; i < predatorImageTotal; i++){
    		...
    		if (!clk->predator.images[i]) return 0;
    	}
	}
