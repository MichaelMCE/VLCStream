...
static inline void drawButtonsVolume (TVLCPLAYER *vp, TCCBUTTONS *btns, TVIDEOOVERLAY *playctrl, TFRAME *frame)
{
	...
	if (buttonsStateGet(btns, VBUTTON_VOLUME)){
		...
		if (volume > 0.0001){ // is not muted
			...
			drawVolume(img, frame, ccGetPositionX(bvol), ccGetPositionY(bvol), 0, (100.0 - volume)*volFactor, img->width, img->height);
		}
...
static inline int page_plyctrlStartup (TVIDEOOVERLAY *plyctrl, TVLCPLAYER *vp, const int fw, const int fh)
{
	...
	ccSetPosition(label, (fw - ccGetWidth(label))/2, ((fh - ccGetHeight(label))/2)+32);
	...
	//ccSetPosition(btn, abs(fw - ccGetWidth(btn)), fh - ccGetHeight(btn) - 10);
	...
	//ccSetMetrics(slider, (fw - ccGetWidth(slider))/2, fh - ccGetHeight(slider) - 1, -1, -1);
	...
	listboxSetLineHeight(listbox, 46);
