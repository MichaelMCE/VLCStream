...
static inline void doRotary (TEQ *eq, rotary_t *enc)
{
	if (enc->encoderId == 1){
...
static inline int page_eqStartup (TEQ *eq, TVLCPLAYER *vp, const int fw, const int fh)
{
	...
	TCCBUTTON *btn = buttonsCreateButton(eq->btns, L"common/back_left92.png", NULL, EQ_BACK, 1, 0, x, y);

	btn = buttonsCreateButton(eq->btns, L"eq/reset.png", NULL, EQ_RESET, 1, 1, x, y);	
