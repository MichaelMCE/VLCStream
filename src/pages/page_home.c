...
static THOMEBTN homeBtns[] = {
	...
	{1, PAGE_PLY_QUEUE,		L"home/playlistqueue.png", NULL, 1, 0},
...
static inline int page_homeRender (TAPPLAUNCHER *al, TVLCPLAYER *vp, TFRAME *frame)
{
	...
	if (!playlistGetTotal(plc)){
		...
		buttonsStateSet(btns, HOME_BTN_ID(PAGE_PLY_QUEUE), 0);
...
//#endif
		...
		buttonsStateSet(btns, HOME_BTN_ID(PAGE_PLY_QUEUE), 1);
...
static void doRotary (TAPPLAUNCHER *al, rotary_t *enc)
{
	if (enc->encoderId == 1){
