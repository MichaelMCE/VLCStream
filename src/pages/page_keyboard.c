...
static int doRotary (TKEYBOARD *vkey, rotary_t *enc)
{
	if (enc->encoderId == 1){
...
static inline int page_vkbInput (TKEYBOARD *vkey, TVLCPLAYER *vp, const int msg, const int flags, void *dataPtr)
{
	switch(msg){
	  ...
	  case PAGE_IN_ROTARY_CHANGE:
	  	return doRotary(vkey, (rotary_t*)dataPtr);
	}
