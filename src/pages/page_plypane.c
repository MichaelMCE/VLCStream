...
static inline void buildTitleBar (TPLYPANE *plypane, TLABEL *title, const int uid)
{
	...
	if (!plypane->timestampItemId){
		int itemId = labelTextCreate(title, "0:00/0:00", 0, PANE_TIMESTAMP_FONT, 0, 0);
	 	labelRenderFilterSet(title, itemId, 0);
...
static inline int plyPaneAddPlaylist (TPLYPANE *plypane, TPANE *pane, TPLAYLISTMANAGER *plm, const int pid)
{
	...
	if (playlistLock(plc)){
		...
		// ensure folders are topmost, add first
...
static inline int page_plypaneInitalize (TPLYPANE *plypane, TVLCPLAYER *vp, const int width, const int height)
{
	...
	plypane->icons.audio = artManagerImageAdd(vp->am, buildSkinD(vp,bufferw,L"pane/audio36.png"));
	plypane->icons.folder = artManagerImageAdd(vp->am, buildSkinD(vp,bufferw,L"pane/folder36.png"));
	plypane->icons.play = artManagerImageAdd(vp->am, buildSkinD(vp,bufferw,L"pane/play36.png"));
	plypane->icons.back = artManagerImageAdd(vp->am, buildSkinD(vp,bufferw,L"pane/back108.png"));
	plypane->icons.art = artManagerImageAdd(vp->am, buildSkinD(vp,bufferw,L"shelf/noart.png"));
	plypane->icons.closePane = artManagerImageAdd(vp->am, buildSkinD(vp,bufferw,L"pane/close108.png"));
