...
static void doRotary (TPLYTV *plytv, rotary_t *enc)
{
	if (enc->encoderId == 1){
...
static inline int page_plytvInput (TPLYTV *plytv, TVLCPLAYER *vp, const int msg, const int flags, void *dataPtr)
{
	switch(msg){
	  case PAGE_IN_TOUCH_DOWN:
	  case PAGE_IN_TOUCH_SLIDE:
	  case PAGE_IN_TOUCH_UP:
	  	plytvTouch((TTOUCHCOORD*)dataPtr, flags, vp);
	  	break;
	  ...
	  case PAGE_IN_ROTARY_CHANGE:
	  	doRotary(plytv, (rotary_t*)dataPtr);
	  	break;
	}
