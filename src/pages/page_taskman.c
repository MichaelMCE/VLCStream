...
// override extracted process icon with the below
static const tm_icon tm_icons[] = {
	...
	MAKE_PARTIAL(					 "waterfox.exe",					"waterfox.png"),
	MAKE_PARTIAL(					 "desktops.exe",					"desktops.png"),
	MAKE_PARTIAL(					 "yt-dlp.exe",						"yt-dlp.png"),
	MAKE_PARTIAL(					 "clock64.exe",						"clock.png"),
...
static inline void taskmanCPUGraphInitalize (TTASKMAN *taskman, TVLCPLAYER *vp, const int width, const int height)
{
	...
	if (!taskman->cpugraph.enabled) return;
...
static inline int page_taskmanShutdown (TTASKMAN *taskman)
{
	...
	if (taskman->folders.windows)
		my_free(taskman->folders.windows);		
	if (taskman->cpugraph.enabled)
