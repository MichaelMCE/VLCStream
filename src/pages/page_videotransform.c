...
static inline TFRAME *newStringVar (TVLCPLAYER *vp, const char *str, const float value)
{
	return lNewString(vp->ml->hw, vp->ml->bpp, 0, VTRANSFORM_FONT, str, value);
}
...
static inline int page_tfRender (TTRANSFORM *tf, TVLCPLAYER *vp, TFRAME *frame)
{
	...
	if (vp->vlc->rotateAngle == 0.0)
		value[SLIDER_ROTATE] = newStringVar(vp, "0.0" , 0.0f);
	else
		value[SLIDER_ROTATE] = newStringVar(vp, "%.1f",-vp->vlc->rotateAngle);
	value[SLIDER_SCALE]      = newStringVar(vp, "%.3f", vp->vlc->scaleFactor);
	value[SLIDER_BLUR]       = newStringVar(vp, "%i"  , vp->vlc->blurRadius);
	value[SLIDER_PIXELIZE]   = newStringVar(vp, "%i"  , vp->vlc->pixelize);
	value[SLIDER_BRIGHTNESS] = newStringVar(vp, "%.2f", vp->vlc->brightness);
	value[SLIDER_CONTRAST]   = newStringVar(vp, "%.2f", vp->vlc->contrast);
	value[SLIDER_SATURATION] = newStringVar(vp, "%.2f", vp->vlc->saturation);
	value[SLIDER_GAMMA]      = newStringVar(vp, "%.2f", vp->vlc->gamma);
...
static void doRotary (TTRANSFORM *tf, rotary_t *enc)
{
	if (enc->encoderId == 1){
