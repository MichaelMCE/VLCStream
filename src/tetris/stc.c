...
void rotateTetromino(StcGame *game, int clockwise)
{
    ...
    for (int i = 0; i < game->fallingBlock.size && i < TETROMINO_SIZE; ++i){
        for (int j = 0; j < game->fallingBlock.size && j < TETROMINO_SIZE; ++j){
            if (clockwise)
    ...
    if (game->fallingBlock.x < 0){
        for (int i = 0; (wallDisplace == 0) && (i < -game->fallingBlock.x); ++i){
            for (int j = 0; j < game->fallingBlock.size; ++j){
    ...
    /* Or check collision with right wall */
		...
		int i = game->fallingBlock.size - 1;
    ...
    for (int i = 0; i < game->fallingBlock.size; ++i){
        for (int j = 0; j < game->fallingBlock.size; ++j){
            if (rotated[i][j] != EMPTY_CELL){
 ...
 */
void gameUpdate (StcGame *game, const int x, const int y, const int blockWidth, const int blockHeight)
{
    long currentTime;
    int timeDelta;
    ...
    /* Draw game state */
    platformRenderGame(game, x, y, blockWidth, blockHeight);
}
