...
void vlc_detachEvents (TVLCCONFIG *vlc, void *func, TVLCPLAYER *vp)
{
	if (vlc->emp){
		for (int i = 0; i < sizeof(mp_events)/sizeof(*mp_events); i++)
			vlc_eventPlayerDetach(vlc, mp_events[i], func/*vlc_eventsCallbackLocked*/, vp);
		vlc->emp = NULL;
	}

	if (vlc->em){
		for (int i = 0; i < sizeof(m_events)/sizeof(*m_events); i++)
			vlc_eventMediaDetach(vlc, m_events[i], func/*vlc_eventsCallbackLocked*/, vp);
		vlc->em = NULL;
	}
}

void vlc_attachEvents (TVLCCONFIG *vlc, void *func, TVLCPLAYER *vp)
{
	vlc_getMediaPlayerEventManager(vlc);
	for (int i = 0; i < sizeof(mp_events)/sizeof(*mp_events); i++)
		vlc_eventPlayerAttach(vlc, mp_events[i], func/*vlc_eventsCallbackLocked*/, vp);

	vlc_getMediaEventManager(vlc);
	for (int i = 0; i < sizeof(m_events)/sizeof(*m_events); i++)
		vlc_eventMediaAttach(vlc, m_events[i], func/*vlc_eventsCallbackLocked*/, vp);
}
			...
			if (t){
				if (t->psz_name){
					if (!*t->psz_name){
						...
						if (name){
							__mingw_snprintf(name, 31, " %i ", i+1);
							title->name = name;
						}else{
							break;
						}
				...
				if (t->i_seekpoint && t->seekpoint){
					...
					for (int j = 0; j < t->i_seekpoint; j++){
						if (t->seekpoint[j]->psz_name){
							...
							if (buffer){
								__mingw_snprintf(buffer, 31, "%i", j);
...
void vlc_inputEventCbDel (TVLCCONFIG *vlc, TVLCPLAYER *vp)
{
	if (vlc->mp){
		...
		if (p_input){
			var_DelCallback(p_input, "intf-event", input_event_changed, vp);
			vlc_object_release(p_input);
		}
	}
}
...
void vlc_inputEventCbSet (TVLCCONFIG *vlc, TVLCPLAYER *vp)
{
	if (vlc->mp){
		input_thread_t *p_input = libvlc_get_input_thread(vlc->mp);
		if (p_input){
			var_AddCallback(p_input, "intf-event", input_event_changed, vp);
			vlc_object_release(p_input);
		}
	}
}
...
int vlc_attachmentsSave (TVLCPLAYER *vp, TVLCCONFIG *vlc)
{
	...
	if (p_input){
		...
		if (p){
			...
			if (p->i_attachment){
				const char *exts[2][5] = {{"jpg", "jpeg", "JPG", "JPEG", NULL}, {"png", "PNG", NULL, NULL, NULL}};				
				char name[MAX_PATH_UTF8+1];
				
				for (int i = 0; i < p->i_attachment; i++){
					char *ext = "";
						
					if (strstr(p->attachment[i]->psz_mime, "jpeg")){
						if (!hasPathExtA(p->attachment[i]->psz_name, exts[0]))
							ext = ".jpg";
					}else if (strstr(p->attachment[i]->psz_mime, "png")){
						if (!hasPathExtA(p->attachment[i]->psz_name, exts[1]))
							ext = ".png";						
					}
					__mingw_snprintf(name, MAX_PATH_UTF8, "%i_%s%s", i+1, p->attachment[i]->psz_name, ext);
					
   					if (writeBin(p->attachment[i]->p_data, p->attachment[i]->i_data, name) > 0){
   						ct++;
   						dbprintf(vp, "Attachment #%i: %s:  '%s'  saved", i+1, p->attachment[i]->psz_mime, name);
   					}else{
   						dbprintf(vp, "Attachment #%i: %s:  '%s'  not saved", i+1, p->attachment[i]->psz_mime, name);
   					}
   				}
			}
...
{
	if (epgevent){
		if (epgevent->psz_name)
			my_free(epgevent->psz_name);
		if (epgevent->psz_short_description)
			my_free(epgevent->psz_short_description);
		if (epgevent->psz_description)
			my_free(epgevent->psz_description);
		my_free(epgevent);
	}
}
...
static inline TVLCEPGEVENT *epg_copyEvent (TVLCEPGEVENT *vlc_epg_event)
{
	...
	if (epgevent){
		epgevent->i_start = vlc_epg_event->i_start;
		epgevent->i_duration = vlc_epg_event->i_duration;
		
		if (vlc_epg_event->psz_name)
			epgevent->psz_name = my_strdup(vlc_epg_event->psz_name);
		if (vlc_epg_event->psz_short_description)
			epgevent->psz_short_description = my_strdup(vlc_epg_event->psz_short_description);
		if (vlc_epg_event->psz_description)
			epgevent->psz_description = my_strdup(vlc_epg_event->psz_description);
	}
	return epgevent;
}
