...
#if ((LIBVLC_VERSION_MAJOR == 2 && LIBVLC_VERSION_MINOR >= 2) || (LIBVLC_VERSION_MAJOR == 3))
#define USEINTERNALEQ			0				// 1:enable internal EQ API for VLC 2.0.x. 0: use API from VLC 2.2.x. VLC 2.1.x isn't supported
#else
#define USEINTERNALEQ			1
#include "libvlceq.h"
#endif


typedef struct {
    int64_t i_start;    /* Interpreted as a value returned by time() */
    int     i_duration;    /* Duration of the event in seconds */
    char    *psz_name;
